...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password !== $repass) {
        ...
        if ($check->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (name, email, address, contact, password, role) VALUES (?, ?, ?, ?, ?, 'farmer')");
            $stmt->bind_param("sssss", $name, $email, $address, $contact, $hashedPassword);

            if ($stmt->execute()) {
                $message = "Farmer account successfully created!";
...
<head>
  ...
  <title>HarvestHub - Farmer Signup</title>
...
<body>
  <div class="login-container">
    ...
    <?php if ($message_type !== "success"): ?>
      <form action="farmer_signup.php" method="post">
        ...
        <button type="submit" class="btn-login">Sign Up as Farmer</button>
      </form>
    <?php endif; ?>
  </div>
</body>
